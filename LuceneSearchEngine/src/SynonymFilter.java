...
import com.giosis.qube.core.analysis.TokenFilter;
...
import com.giosis.qube.core.analysis.tokenattributes.OffsetAttribute;
import com.giosis.qube.core.analysis.tokenattributes.PositionIncrementAttribute;
import com.giosis.qube.core.analysis.tokenattributes.PositionLengthAttribute;
import com.giosis.qube.core.analysis.tokenattributes.TypeAttribute;
import com.giosis.qube.core.store.ByteArrayDataInput;
import com.giosis.qube.core.util.ArrayUtil;
import com.giosis.qube.core.util.AttributeSource;
...
import com.giosis.qube.core.util.RamUsageEstimator;
import com.giosis.qube.core.util.fst.FST;
import com.giosis.qube.extend.analysis.tokenattributes.LanguageAttribute;
import com.giosis.qube.util.Tool;

/**
 * Qube용 유사어 필터<br>
 * toLowerCase -> toUpperCase로 변경외 나머지 동일
 * tcpark 2017-04-12
 * 
...
public final class QubeSynonymFilter extends TokenFilter {

  public static final String TYPE_SYNONYM = "SYNONYM";

  private final QubeSynonymMap synonyms;
  ...
  private final LanguageAttribute langAtt = addAttribute(LanguageAttribute.class);
   ...
   * @param ignoreCase case-folds input for matching with {@link Character#toUpperCase(int)}.
   *                   Note, if you set this to true, it's your responsibility to lowercase
   *                   the input entries when you create the {@link SynonymMap}
   */
  public QubeSynonymFilter(TokenStream input, QubeSynonymMap synonyms, boolean ignoreCase) {
    super(input);
  ...
  */
  ...
  private void parse() throws IOException {
    ...
    while(true) {
      ...
      while(bufUpto < bufferLen) {
        ...
        if (fst.findTargetArc(ignoreCase ? Character.toUpperCase(codePoint) : codePoint, scratchArc, scratchArc, fstReader) == null) {
  ...
  /**
   * 유사어 Term 및 Language 정보를 셋팅하는 내용
 * @author : yonghw
 * 2019. 9. 11. - 
 */
public void copyTermAtt(CharsRef output) {
	  String[] o = Tool.splitLastIndex(QubeSynonymMap.SYNO_LANG_SPLIT, String.valueOf(output.chars,
			  output.offset, output.length));
	  output = new CharsRef(o[QubeSynonymMap.SYNONYM_IDX]);
	  langAtt.setLang(o[QubeSynonymMap.LANGUAGE_IDX].trim());
