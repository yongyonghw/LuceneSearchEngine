 ...
 */
package com.giosis.qube.extend.analysis;
...
import com.giosis.qube.core.analysis.Analyzer;
import com.giosis.qube.core.analysis.TokenStream;
import com.giosis.qube.core.analysis.synonym.SynonymMap;
import com.giosis.qube.core.analysis.tokenattributes.CharTermAttribute;
import com.giosis.qube.core.analysis.tokenattributes.PositionIncrementAttribute;
import com.giosis.qube.core.store.ByteArrayDataOutput;
import com.giosis.qube.core.util.BytesRef;
import com.giosis.qube.core.util.BytesRefBuilder;
import com.giosis.qube.core.util.BytesRefHash;
import com.giosis.qube.core.util.CharsRef;
import com.giosis.qube.core.util.CharsRefBuilder;
import com.giosis.qube.core.util.IntsRefBuilder;
import com.giosis.qube.core.util.fst.ByteSequenceOutputs;
import com.giosis.qube.core.util.fst.FST;
import com.giosis.qube.core.util.fst.Util;
import com.giosis.qube.util.Tool;

/**
 * {@link SynonymMap} A map of synonyms, keys and values are phrases.
 * 
 * @lucene.experimental SynonymMap 수정을 위해 분리함
 */
public class QubeSynonymMap {

	/**
	 *  유사어:국가 를 구분하는 구분자 ex) nike:EN,나이키:KO ...
	 */
	public static final String SYNO_LANG_SPLIT = ":";
	
	public final static int SYNONYM_IDX = 0;
	public final static int LANGUAGE_IDX = 1;
	...
	public QubeSynonymMap(FST<BytesRef> fst, BytesRefHash words, int maxHorizontalContext) {
		this.fst = fst;
		this.words = words;
	 ...
	 * <p>
	 * Call add() until you have added all the mappings, then call build() to
	 * get an FSTSynonymMap
	 * 
	 * @lucene.experimental
	 */
	public static class Builder {
		...
		/**
		 * If dedup is true then identical rules (same input, same output) will
		 * be added only once.
		 */
		 ...
		 * {@link QubeSynonymMap#WORD_SEPARATOR}. reuse and its chars must not
		 * be null.
		 */
		public static CharsRef join(String[] words, CharsRefBuilder reuse) {
			...
			for (String word : words) {
				...
				final int needed = (0 == upto ? wordLen : 1 + upto + wordLen); // Add
																				// 1
																				// for
																				// WORD_SEPARATOR
				...
				if (upto > 0) {
					buffer[upto++] = QubeSynonymMap.WORD_SEPARATOR;
				}
		...
		private boolean hasHoles(CharsRef chars) {
			...
			for (int idx = chars.offset + 1; idx < end; idx++) {
				if (chars.chars[idx] == QubeSynonymMap.WORD_SEPARATOR
						&& chars.chars[idx - 1] == QubeSynonymMap.WORD_SEPARATOR) {
					return true;
				}
			}
		...
		private void add(CharsRef input, int numInputWords, CharsRef output, int numOutputWords, boolean includeOrig) {
			...
			// + numInputWords + " output=" + output + " numOutputWords=" +
			// numOutputWords);
			...
			// 맵엔트리 구성은 input으로 셋팅 by yonghw 20190905
			String inputString = String.valueOf(input);
			String outputString = String.valueOf(output);
			// 사전 값을 분리한다. ex) nike:en 
			
			String[] inputArr = Tool.splitLastIndex(SYNO_LANG_SPLIT, inputString);
			String[] outputArr = Tool.splitLastIndex(SYNO_LANG_SPLIT, outputString);
			
			// 분리된 사전 값중 키워드만 취한다.
			CharsRef splittedInput = new CharsRef(inputArr[SYNONYM_IDX]);

			// 키워드를 키로 하는 MapEntry를 구성한다.
			MapEntry e = workingSet.get(splittedInput);
			if (e == null) {
				e = new MapEntry();
				workingSet.put(CharsRef.deepCopyOf(splittedInput), e); 
				//make a copy,since we will keep around in our map
			}

			// 입력의 유사어 서수는 포함하지 않도록 추가
			if (!inputArr[SYNONYM_IDX].equals(outputArr[SYNONYM_IDX]))
				e.ords.add(ord);
		...
		private int countWords(CharsRef chars) {
			...
			while (upto < limit) {
				if (chars.chars[upto++] == QubeSynonymMap.WORD_SEPARATOR) {
					wordCount++;
				}
			}
			return wordCount;
		...
		/**
		 * Add a phrase-&gt;phrase synonym mapping. Phrases are character
		 * sequences where words are separated with character zero (U+0000).
		 * Empty words (two U+0000s in a row) are not allowed in the input nor
		 * the output!
		 * 
		 * @param input
		 *            input phrase
		...
		/**
		 * Builds an {@link QubeSynonymMap} and returns it.
		 */
		public QubeSynonymMap build() throws IOException {
			ByteSequenceOutputs outputs = ByteSequenceOutputs.getSingleton();
			// TODO: are we using the best sharing options?
			com.giosis.qube.core.util.fst.Builder<BytesRef> builder = new com.giosis.qube.core.util.fst.Builder<>(
			...
			for (int keyIdx = 0; keyIdx < sortedKeys.length; keyIdx++) {
				...
				int estimatedSize = 5 + numEntries * 5; // numEntries + one ord
														// for each entry
				...
				// System.out.println(" add input=" + input + " output=" +
				// scratch + " offset=" + scratch.offset + " length=" +
				// scratch.length + " count=" + count);
				builder.add(Util.toUTF32(input, scratchIntsRef), scratch.toBytesRef());
			}

			FST<BytesRef> fst = builder.finish();
			return new QubeSynonymMap(fst, words, maxHorizontalContext);
		}
	}
		...
		public CharsRef analyze(String text, CharsRefBuilder reuse) throws IOException {
			try (TokenStream ts = analyzer.tokenStream("", text)) {
				...
				while (ts.incrementToken()) {
					...
					reuse.grow(reuse.length() + length
							+ 1); /* current + word + separator */
					int end = reuse.length();
					if (reuse.length() > 0) {
						reuse.setCharAt(end++, QubeSynonymMap.WORD_SEPARATOR);
